mavikentApp.controller("RoomServiceCtrl",function($scope,$http,$rootScope ,$interval){
    $scope.istekler=[];
    $interval(function(){
          $http.post(host+"/api/service/search?token="+$rootScope.mkb.token,{floor_no : $rootScope.mkb.current_user.floor_id.name,isActive:"false"}).success(function(resp){
    ...
    $scope.istekAl=function(item,index){
        $http.put(host+"/api/service?token="+$rootScope.mkb.token,{_id : item._id, isActive : true}).success(function(resp){
